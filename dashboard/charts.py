...
_AXIS = dict(
    showgrid=True,
    gridcolor=config.GRIDLINE,
    gridwidth=1,
    zeroline=False,
    linecolor=config.AXIS_LINE,
    tickfont=dict(color=config.INK_MUTED, size=11, family=config.FONT_MONO),
    title_font=dict(color=config.INK_MUTED, size=11, family=config.FONT_MONO),
)


def _base_layout(height: int = 320, showlegend: bool = False, **kwargs) -> Dict[str, Any]:
    layout = dict(
        height=height,
        margin=dict(l=8, r=8, t=28, b=8),
        paper_bgcolor='rgba(0,0,0,0)',
        plot_bgcolor=config.SURFACE_CHART,
        font=dict(family=config.FONT_MONO, color=config.INK_SECONDARY, size=11),
        showlegend=showlegend,
        legend=dict(
            orientation='h', yanchor='bottom', y=1.02, xanchor='left', x=0,
            font=dict(color=config.INK_SECONDARY, size=11),
            bgcolor='rgba(0,0,0,0)',
        ),
        hoverlabel=dict(
            bgcolor=config.SURFACE_CARD,
            bordercolor=config.BORDER,
            font=dict(family=config.FONT_MONO, color=config.INK_PRIMARY, size=11),
        ),
        xaxis=dict(_AXIS),
        yaxis=dict(_AXIS),
    )
...
def empty_figure(message: str = 'No data yet', height: int = 320) -> go.Figure:
    ...
    fig.add_annotation(
        ...
        showarrow=False,
        font=dict(family=config.FONT_MONO, size=13, color=config.INK_MUTED),
    )
    return fig
...
def equity_curve(equity_df: pd.DataFrame, height: int = 340) -> go.Figure:
    ...
    if equity_df is None or equity_df.empty:
        return empty_figure('No equity snapshots yet. The engine writes one every 15 min.', height)

    df = equity_df.dropna(subset=['equity']).sort_values('ts')
    if df.empty:
        return empty_figure('No equity snapshots yet.', height)
    ...
    fig.add_hline(
        y=start, line=dict(color=config.INK_MUTED, width=1, dash='dash'),
        annotation_text='start {:,.2f}'.format(start),
        annotation_position='top left',
        annotation_font=dict(color=config.INK_MUTED, size=10, family=config.FONT_MONO),
    )
    fig.update_layout(**_base_layout(height=height, hovermode='x unified'))
    # Crosshair: an equity curve is read by hovering, so ship the spike.
    fig.update_xaxes(showspikes=True, spikemode='across', spikethickness=1,
                     spikecolor=config.INK_MUTED, spikedash='dot')
    fig.update_yaxes(title_text='equity', tickformat=',.2f',
                     rangemode='normal', autorange=True)
    return fig


def drawdown_curve(equity_df: pd.DataFrame, height: int = 180) -> go.Figure:
    ...
    """
    if equity_df is None or equity_df.empty:
        return empty_figure('No equity history to draw down from.', height)

    df = equity_df.dropna(subset=['equity']).sort_values('ts')
    if len(df) < 2:
        return empty_figure('Need at least two snapshots to compute drawdown.', height)
...
def pnl_distribution(trades: pd.DataFrame, height: int = 300) -> go.Figure:
    ...
    if trades is None or trades.empty:
        return empty_figure('No closed trades yet.', height)
    ...
    if pnl.empty:
        return empty_figure('No closed trades with a recorded PnL yet.', height)
    ...
    fig.add_vline(x=0, line=dict(color=config.INK_MUTED, width=1))
...
def strategy_pnl_bar(perf: pd.DataFrame, height: Optional[int] = None) -> go.Figure:
    ...
    if perf is None or perf.empty:
        return empty_figure('No strategies with trades yet.', height or 300)

    df = perf.copy()
    df['pnl_net'] = pd.to_numeric(df['pnl_net'], errors='coerce').fillna(0.0)
    df = df[df['total_trades'].fillna(0) > 0]
    if df.empty:
        return empty_figure('No strategy has closed a trade yet.', height or 300)
    ...
    fig = go.Figure(go.Bar(
        ...
        orientation='h',
        marker=dict(color=colors, line=dict(color=config.SURFACE_CHART, width=2)),
        text=labels, textposition='outside',
        textfont=dict(family=config.FONT_MONO, size=11, color=config.INK_SECONDARY),
        customdata=df[['total_trades']].values,
        hovertemplate='%{y}<br>net pnl %{x:,.2f}<br>%{customdata[0]} closed trades<extra></extra>',
        cliponaxis=False,
    ))
    fig.update_layout(**_base_layout(height=height))
    fig.add_vline(x=0, line=dict(color=config.AXIS_LINE, width=1))
...
def verdict_composition(counts: Dict[str, Any], height: int = 120) -> go.Figure:
    ...
    """
    if not counts:
        return empty_figure('No graveyard summary available.', height)
    ...
    for i, k in enumerate(keys):
        ...
        fig.add_trace(go.Bar(
            ...
            marker=dict(color=palette.get(k, config.CATEGORICAL[i % len(config.CATEGORICAL)]),
                        line=dict(color=config.SURFACE_CHART, width=2)),
            hovertemplate='{}<br>%{{x:,.0f}} rows ({:.2%})<extra></extra>'.format(k, share),
        ))

    fig.update_layout(**_base_layout(height=height, showlegend=True, barmode='stack'))
    fig.update_xaxes(visible=False)
    fig.update_yaxes(visible=False)
    return fig


def pass_concentration_bar(items: List[Dict[str, Any]], height: Optional[int] = None) -> go.Figure:
    """Where the PASS rows pile up. Single hue: bar length is the encoding, so
    coloring by value too would be redundant ink."""
    if not items:
        return empty_figure('No pass concentration data.', height or 240)
    ...
    fig = go.Figure(go.Bar(
        ...
        marker=dict(color=config.CATEGORICAL[0], line=dict(color=config.SURFACE_CHART, width=2)),
...
def strategy_firing_bar(health: pd.DataFrame, top_n: int = 20,
    ...
    if health is None or health.empty:
        return empty_figure('No judge evidence pack available.', height or 300)
    ...
    df = df[tested > 0].copy()
    if df.empty:
        return empty_figure('No strategy has any tested rows.', height or 300)
    ...
    labels = ['NON-FIRING' if s else '{:,.0f} trades'.format(t)
              for s, t in zip(silent, df['trades'])]
    ...
    fig = go.Figure(go.Bar(
        ...
        marker=dict(color=colors, line=dict(color=config.SURFACE_CHART, width=2)),
        text=labels, textposition='outside',
        textfont=dict(family=config.FONT_MONO, size=10, color=config.INK_SECONDARY),
...
def asset_class_pnl_bar(breakdown: List[Dict[str, Any]], height: Optional[int] = None) -> go.Figure:
    """Pooled PnL per trade by strategy x asset class, from the judge pack."""
    if not breakdown:
        return empty_figure('No asset class breakdown in the judge pack.', height or 320)
    ...
    if not rows:
        return empty_figure('No usable pnl_per_trade values.', height or 320)
...
def lifecycle_timeline(events: pd.DataFrame, height: Optional[int] = None) -> go.Figure:
    ...
    if events is None or events.empty:
        return empty_figure('No promotion or demotion events logged yet.', height or 240)

    df = events.dropna(subset=['time']).copy()
    if df.empty:
        return empty_figure('No promotion or demotion events with a timestamp.', height or 240)
    ...
    if not fig.data:
        return empty_figure('No lifecycle events to plot.', height)

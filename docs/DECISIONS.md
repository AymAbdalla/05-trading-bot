...
### D-380. tick 7 rulings: 048 findings ratified, implementation HELD; 049 accepted and BUILT; 042 form amendment accepted (RAVEN ruling, 2026-08-20)

**Numbering provenance.** Originally drafted as D-358; consumed by the drawdown-resume AYM ruling (D-358). An attempted D-363 assignment collided with Aym's unconstrained-measurement rulings. An attempted D-365 assignment collided with Aym's per-trade order-size ruling. Recorded as D-380 per the D-380+ reserved-block convention (Raven, 2026-08-20): queued Raven-recorded rulings live at D-380+ so Aym's live rulings, which land in the low range faster than briefs can be written, never collide with them again. Three collisions, five dispatch attempts, and two STOPPED-at-the-gate sessions preceded this record; they are committed alongside it as collision documentation.

**Problem being ruled on.** The forge reasoner's tick 7 produced two proposals and one amendment. (1) 048 shows the portfolio drawdown that halted the main book twice is computed from incompatible accounting universes: the numerator (`PolymarketPaperAdapter.get_equity()`) is process-local and re-bases to exactly 1000.00 on every restart, while the denominator (`SELECT MAX(equity) FROM equity_snapshots`) is process-global; the re-base credit summed to 1,473.53 USD across 16 epochs in env A, and the all-time peak 1027.9641 overstated the realised book by 891.72 USD. A second defect: `get_equity()` subtracts `capital_at_risk()`, so open premium reads as drawdown and blocking entries mechanically cures the trigger. (2) 049 shows the 40% line is a clock, not a control: both books bled at ~-29.5 USD/h and would reach a 400 USD drawdown in ~13.6-13.8h; the breach is 0.36 sigma beyond the epoch-mean path; 54.5% of the epoch's net loss came from strategies already TESTED_FAILED (fair_value_arb family) or already killed by D-356 R4 (dip_arb), still trading only because convention 13 makes kills inert until a restart. (3) 042's amendment records the venue fee-surface movement (taker rebates up to 50%, maker rebates up to 25%, special-user 80% rebate) and refers a FORM question: state the CONFIRM branch as a fraction of the measured taker fee, not an absolute -0.0100/share constant.

R1. **Proposal 048: FINDINGS RATIFIED in full; the FORK and all implementation are HELD.** The two measurement defects are confirmed (deterministic, reproduced read-only; no sample-size argument needed). The epoch-scoped-vs-book-lifetime fork was REFERRED to Raven AND Aym by 048 itself ("the choice is not mine"). **Context update at recording time:** the drawdown decision has since landed as D-358 (resume, keep measuring, fund-if-zero) and D-359 (auto-halt disabled in shadow). Aym's decision was to keep the current instrument and re-fund at zero, NOT to change the measurement. 048's implementation therefore stays HELD exactly as this ruling directs, and the hold is now a settled position rather than a wait: no `_risk_equity_state` epoch scoping, no three-named-drawdown payload, no fixture, no stale-sentence correction at shadow_loop.py:399 / constraints.py:237, and 048's kill condition is NOT touched. Amend 048 with a dated ruling note only. The findings are recorded here so nothing is lost if the paper trail moves.

R2. **Proposal 049 is ACCEPTED and BUILT, exactly as written.** It changes no limit, no halt state, no restart, no strategy; it adds one read-only reporter and one informational field on future `portfolio_drawdown` payloads. Build `backtest/drawdown_attribution.py` per 049's kill condition, all four holds: (1) per-epoch uptime/rate/hourly mean-sd/hours-to-limit/sigma with MARKET-SIDE cluster counts beside every close count (046 - do not print a close count alone); (2) composition by `strategy_id` and `exit_reason` with losers and winners as SEPARATE subtotals, explicitly labelled an ACCOUNTING decomposition, and it must NOT compute a book-without-strategy-X number (049 rule 4); (3) every future `portfolio_drawdown` breach writes sigma and hours-to-limit into its `risk_events` payload; (4) harness 21/21 rc 0 and the full suite green with a count re-derived in-session (never quote an inherited baseline - the suite has grown; re-derive, convention 25). Each database is treated separately (convention 32); no maker/taker pooling; no counterfactual computation. The 042 maker census (87 env A / 0 env B) stays NOT_TESTED and ungraded: the markout instrument does not exist yet.

R3. **Proposal 042's amendment is ACCEPTED as written, and the referred FORM question is RULED: express CONFIRM as a fraction of the measured taker fee.** The amendment (prose, no frontmatter change, no threshold re-size) is ratified. The FORM change is accepted: the CONFIRM branch's absolute -0.0100/share boundary becomes a fraction of the measured taker fee (the derivation in the amendment: a 25%-of-matched-taker-fees schedule yields at most 0.25 x 0.0315 = 0.0079/share, so an absolute -0.0100 can invert the band). Amendment text only - no code, no constant re-sized, and the unverified-external-claim rule applies: no ceiling moves on press-reported numbers. Same for the retire-vs-confirm decision: NOT made here, the probe stays live.

R4. **Context ratified, items recorded, no action.** (a) env B's `market_tape` had written nothing since 2026-08-19T07:27:41Z despite the loop running - recorded as an open item for the keying-restart window, NOT investigated by the ruling session. *Recording note: subsequently root-caused and actioned outside this ruling - D-362 R4 moved the tape writer out of `dip_arb` into `shadow_loop` so the tape is written every cycle regardless of roster, and D-363 R4 made the tape FULL including crypto. This item is closed by those rulings, not by this one.* (b) `temporal_arbitrage` lifetime re-derived: env A +7.35 on 232, env B -17.81 on 199; standing correction 1 updated accordingly; never quote -0.0006. (c) The per-event-cap question the brief asked is MEASURED and REFUSED in 049 as confounded (the cap binds where the book has most signals; 330 closes ~= 15 market-sides); ratify the refusal, do not re-measure. (d) 043's opposite-sign readings across the two books continue (5th reading) - exactly the clustered-sampling behaviour 046 diagnosed; band and bar NOT re-sized (D-354 R2 holds). (e) The tick 7 cycle's `external-signals-2026-08-20-cycle6.md` is part of the cycle output and is committed with it.

**Recording-session note.** Recorded by `cody-tick7-completion` (PID 41541) on 2026-08-20 at ~14:37 EDT / 18:37 UTC, measured with `date`, at HEAD `e9af01a` re-derived with `git rev-parse HEAD`. Gate at recording: the D-366 session (PID 38391) confirmed DEAD with its handoff `docs/handoffs/2026-08-20-D366-position-pct.md` present; `docs/handoffs/from-raven/.lock` absent, then claimed; no live claude sibling working this repo (tmux server 37068 is the known stale-argv trap, `claude mcp serve` processes are MCP servers); all three shadow loops LIVE and untouched (main 40841, env B 40884, realm C 40927). `AGENT_ID` measured EMPTY in a python subprocess; commits declared through the sanctioned `CONFLICT_CHECK_AGENT_ID` channel. **Implementation caveat on R2 hold 3, recorded rather than discovered later:** `SHADOW_RISK_LIMITS` sets `max_drawdown_frac=1.0` (D-359, A-17), and `drawdown_frac()` is bounded above by 1.0, so the `portfolio_drawdown` constraint CANNOT fire in shadow mode today. The enriched payload is therefore correct but DORMANT on all three shadow books; it becomes live only under the real-money `DEFAULT_LIMITS` (0.25) or if a future ruling lowers the shadow limit. The 5-breach bar in 049's kill condition will not advance while the shadow limit stands at 1.0.

### D-381. D-362 execution review rulings, amended for the D-363 supersession (RAVEN ruling, 2026-08-20)

**Numbering provenance.** Originally drafted as D-366 in the tick7-completion brief; D-366 was displaced first by the D-365 consumption and then taken outright by Aym's percentage position-cap ruling. Recorded as D-381 under the same D-380+ reserved-block convention as D-380.

**Problem being ruled on.** The D-362 execution session raised four review questions: whether proceeding past the freeze gate with an idle sibling alive was sound, whether `market_tape` should keep excluding crypto, whether the env B roster overlap with main was as briefed, and whether `docs/handoffs/from-raven/` should stay gitignored. Two of the four answers were overtaken by Aym's D-363 rulings before they could be recorded. **History is recorded honestly here: D-364 R3 already ruled that D-363 wins the contradictions, and nothing below contradicts D-363.**

R1. **Freeze-gate call: ruled SOUND, and the standing rule is codified. STILL LIVE.** Proceeding under idle PID 438 (`cody-tick7-rulings`) was correct: it was idle (73s CPU over 7.5 hours), its scope was disjoint, Raven dispatched the session knowing it existed, and the 16-path pathspec commit was verified. **Standing rule: the freeze gate blocks only an ACTIVE sibling working this repo and scope; an idle parked sibling does not block, but the decision to proceed MUST be documented in the handoff.** An active sibling - one burning CPU on this repo, or holding `docs/handoffs/from-raven/.lock` with a live PID - is a hard stop.

R2. **`market_tape` crypto exclusion: originally KEPT by the D-362 review; SUPERSEDED by D-363 R4. HISTORICAL.** The review's answer was to keep the exclusion. Aym then ruled the opposite: full market tape, all windows, crypto included, "no reason to limit it." **D-363 R4 governs. The exclusion is gone.** Recorded here only so the review's answer is not silently lost.

R3. **env B roster overlap (11 vs 16 strategies, 7 overlapping): originally CONFIRMED as briefed by the D-362 review; SUPERSEDED by D-363 R5. HISTORICAL.** The review confirmed the overlap was real and as described. Aym then ruled that no strategy runs in more than one realm. **D-363 R5 governs: the registry is a disjoint partition** - main 16, env B 4, realm C 6, 26 strategies, no overlap. Recorded here only so the review's confirmation is not silently lost.

R4. **`docs/handoffs/from-raven/` gitignore: SETTLED, stays gitignored. STILL LIVE.** Briefs are internal working instructions, not project artifacts; keeping them out of the tree is consistent with the standing gitignore rule. Handoffs written BY a session (`docs/handoffs/*.md`) are committed; briefs written TO a session are not.

**Recording-session note.** Recorded by `cody-tick7-completion` in the same session and commit as D-380; same gate evidence. R2 and R3 are recorded as HISTORICAL and are superseded in force by D-363 R4 and D-363 R5 respectively, per D-364 R3. R1 and R4 remain in force.

### D-382. Confidence-based position sizing (supersedes the $10 flat order size) (AYM RULING, 2026-08-20)

Aym, replying to Raven's ruling #1 ("$10 order size stays"): "i disagree, 10 is too low, the strategies should size up based on confidence and winning percentage, and not exceed 90% per trade."

R1. The $10 flat order size is REPLACED by confidence-based sizing. Strategies size UP based on confidence and winning percentage.
R2. Hard ceiling: never exceed 90% of available capital per trade (D-366's max_position_pct stays as the ceiling).
...
### D-387. Architecture fix + ponytail scope + dispatch broker + Raven output format (AYM rulings, 2026-08-20)

R1. ADOPT the "How to Fix the Architecture" principles for the Raven layer: (a) schema over free-form text — Raven outputs structured data (strict JSON/sections) not natural-language paragraphs when talking to the worker; (b) shorten the chain — strip Raven's extra tools, narrow to a routing/judgment job; (c) deterministic — move repetitive logic into standard code/fixed routing scripts (the dispatch broker IS this). Apply lowest-level-of-approval review to Raven specifically.
R2. PONYTAIL SCOPE (Aym): steal the principles into the brief template NOW. Trial the actual plugin on 06-career-agent AFTER the freeze (post-08-26). Global adoption only if the trial is boring. No plugin install until then.
R3. DISPATCH BROKER: BUILD IT (Cody). scripts/dispatch.py + dispatch_precheck.py + next_decision.py per cody-interface spec. It is the deterministic fix for the duplicate/ghost/stale-lock class. See from-raven/2026-08-20-dispatch-broker.md.
R4. RAVEN OUTPUT FORMAT (Aym): structured, informative, clean, organized. No messy paragraphs. Labeled sections (FACTS/ANALYSIS/RECOMMENDATION), bullets, tables where useful. This is a standing output requirement, not a one-time style.
R5. RAVEN WRITE ACCESS: KEEP, do not remove. Aym: "i do want you to do that, its just the times you do it and when you do it, the judgment of when." So: Raven keeps commit/restart ability BUT must run dispatch_precheck.py first and exercise timing judgment. Refines the cody-interface red list — it is not adopted wholesale.
R6. SHARE RAVEN OUTPUT WITH CODY (Aym): Aym's messages + Raven's replies go to Cody so he sees the full loop ("part of the problem is the webhooks"). Mechanism TBD — likely a per-day summary file Cody reads at session start. Add to A-25 scope.
R7. DOUBLE-REPLY INCIDENT (Aym 08-20): Raven replied twice with the same content, different formatting, to one message. Same duplicate class at the output layer. Root-cause into the output pipeline (webhook double-trigger), fix with the broker + single-output discipline.

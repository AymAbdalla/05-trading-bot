...
def test_the_first_eight_registry_slots_did_not_move():
    ...
    # 26 since `PM_weather_bracket_width_matched` was APPENDED at index 23
    # (proposal 033), `PM_fair_value_settlement_exit` at index 24
    # (proposal 034), `PM_fair_value_mirror_fade` at index 25 (D-326), after
    # `PM_longshot_fade_hold_to_resolution` at index 22 (proposal 032), after
    # `PM_status_quo_collector` at index 21 (proposal 028), after
    # `PM_smart_money_callers` at index 20 (proposal 027), after
    # `PM_maker_rebate_quote_ladder` at index 19 (proposal 024). The prefix
    # pin above is what protects the historical log positions; the total is
    # free to grow as long as nothing before it moves.
    assert len(names) == 26

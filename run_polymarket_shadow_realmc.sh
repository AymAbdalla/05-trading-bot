...
#
#   STARTING_EQUITY="$(<last measured get_equity() for THIS db>)" ./run_polymarket_shadow_realmc.sh
#
...
# as `./<launcher> --equity <number>` - this one with 768.7537. Every one of
...
if [ "$#" -gt 0 ]; then
    die "this launcher takes no arguments (got: $*). It is configured by" \
        ...
        "STARTING_EQUITY=<usd> ./run_polymarket_shadow_realmc.sh"
fi

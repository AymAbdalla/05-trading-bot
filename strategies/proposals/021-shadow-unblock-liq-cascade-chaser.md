...
thesis: "PM_liq_cascade_chaser has never evaluated its own entry condition in the shadow loop: it skipped 10304 of 17446 evaluations on 'liquidation_feed_stale', which is a missing input (a live liquidation recorder (newest row older than stale_after_sec)) rather than a false condition. Supplying that input is what turns this strategy from NOT_TESTED into testable."
...
data_requirements: "BLOCKER: a live liquidation recorder (newest row older than stale_after_sec). Measured over 17446 live shadow evaluations, 66.7% of skips were data-blocked. Until that input exists this strategy is NOT_TESTED (convention 11) and must not be reported as having looked and declined."
...
Source: `db/trading.db` `signals`, read by `agents/forge_shadow_eval.py`. Session covers 386172 decision rows across 21 strategies.
...
| evaluations | 17446 |
| entries | 0 |
| skips classed DATA_BLOCKER | 11638 |
| skips classed GENUINE | 5808 |

Dominant skip reason: `liquidation_feed_stale` (10304 of 17446).
